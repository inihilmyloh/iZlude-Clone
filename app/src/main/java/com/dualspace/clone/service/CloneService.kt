package com.dualspace.clone.service
...
import com.dualspace.clone.core.CloneManager
...
class CloneService : Service() {
    ...
    companion object {
        const val ACTION_CLONE = "com.dualspace.clone.ACTION_CLONE"
        const val ACTION_REMOVE = "com.dualspace.clone.ACTION_REMOVE"
        ...
        serviceScope.launch {
            when (action) {
                ACTION_CLONE -> {
                    ...
                    // Broadcast result
                    sendBroadcast(Intent("com.dualspace.clone.CLONE_COMPLETE").apply {
                        putExtra(EXTRA_PACKAGE_NAME, packageName)
                    })
                }
                ACTION_REMOVE -> {
                    cloneManager.removeClone(packageName)
                    sendBroadcast(Intent("com.dualspace.clone.CLONE_REMOVED").apply {
                        putExtra(EXTRA_PACKAGE_NAME, packageName)
                    })
                }
            }
